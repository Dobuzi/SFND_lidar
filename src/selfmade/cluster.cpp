...
void proximity(const std::vector<std::vector<float>>& points, int i, std::vector<int> *cluster, KdTree* tree, float distanceTol, std::unordered_set<int> *processed)
{
	processed->insert (i); // mark point as processed
	...
	{
		if (processed->find(nearby[j]) == processed->end()) // If point has not been processed
		{
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol, int minSize, int maxSize)
{
	...
	std::unordered_set<int> processed; // processing marker
	std::vector<int> cluster; // Define cluster

	for (int i = 0; i < points.size (); i++) // Iterate through each point
	{	
		if (processed.find(i) == processed.end()) // If point has not been processed
		{
